...
func TestEither(t *testing.T) {
	...
	child := func(t testing.TB) {
		ctx := signals.Trap(context.Background(), os.Interrupt, syscall.SIGQUIT)
		// Should not have received signal
		select {
		case <-ctx.Done():
			is.Fail() // context shouldn't be cancelled yet
...
func TestMultiple(t *testing.T) {
	...
	child := func(t testing.TB) {
		ctx1 := signals.Trap(context.Background(), os.Interrupt)
		ctx2 := signals.Trap(context.Background(), os.Interrupt)
		// Should not have received signal
		select {
		case <-ctx1.Done():
